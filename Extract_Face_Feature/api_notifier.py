...
def notify_main_api(user_id: int, request_id: int):
    try:
        # 로컬 테스트 환경: Docker 컨테이너에서 호스트 PC로 접근할 때 사용
    ...
    #     # 추후 인스턴스 업데이트이후(<MAIN_API_PUBLIC_IP>를 변경해야 함)
    #     response = requests.post("http://<MAIN_API_PUBLIC_IP>:8000/run-recommendation/", json={
    #         "user_id": user_id,
    #         "request_id": request_id
    #     })
    #     print("[INFO] Main API 호출 완료:", response.status_code)
